...
def _get_strided(waveform: Tensor, window_size: int, window_shift: int, snip_edges: bool):
...
def _get_log_energy(strided_input: Tensor, epsilon: Tensor, energy_floor: float):
...
def _get_waveform_and_window_properties(
    ...
    if size_mode == 'time':
        ...
        window_shift = int(frame_shift)
        window_size = int(frame_length)
...
def _subtract_column_mean(tensor: Tensor, subtract_mean: bool):
...
def spectrogram(
    ...
    waveform, window_shift, window_size, padded_window_size = _get_waveform_and_window_properties(
        waveform, channel, sample_frequency, frame_shift, frame_length, round_to_power_of_two, preemphasis_coefficient, size_mode
    )
    ...
    if return_fft:
        ...
        phase = torch.atan2(fft.imag, fft.real)
        mag = fft.abs()**2
        power_spectrum = torch.max(mag, epsilon).log()  # size (m, padded_window_size // 2 + 1)
...
def inverse_mel_scale_scalar(mel_freq: float):
    return 700.0 * (math.exp(mel_freq / 1127.0) - 1.0)


def inverse_mel_scale(mel_freq: Tensor):
    return 700.0 * ((mel_freq / 1127.0).exp() - 1.0)


def mel_scale_scalar(freq: float):
    return 1127.0 * math.log(1.0 + freq / 700.0)


def mel_scale(freq: Tensor):
...
def fbank(
    ...
    if return_fft:
        phase = torch.atan2(fft.imag, fft.real)
        mag = fft.abs()**2
        return mel_energies, (mag,phase)
    else:
        return mel_energies



def _get_dct_matrix(num_ceps: int, num_mel_bins: int):
...
def _get_lifter_coeffs(num_ceps: int, cepstral_lifter: float):
