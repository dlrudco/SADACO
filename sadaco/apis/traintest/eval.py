...
def test_merge_epoch(
    ...
    verbose: bool = True,
    preprocessing : Callable = None,
    target_classes: Union[List, Tuple] = None,
)-> Optional[Union[DefaultDict, np.ndarray]]:
    model.eval().to(device)
    test_loss = 0
    with tqdm(
        ...
        for bidx, batch_info in pbar:
            ...
            if torch.isnan(inputs['input']).any():
                print(f'NaN mag!!! val after preproc')
            output = model(inputs['input'])
            loss = criterion(**{**inputs,'output':output[:, target_classes], 'label':batch_info['label'][:, target_classes]}).mean()
            if torch.isnan(loss):
                breakpoint()
            if criterion.reduction in ['mean', 'none'] :
                test_loss += loss.item() * output.shape[0]
                ...
                test_loss += loss.item()
            pbar.set_postfix(loss=loss.item()/(bidx+1))
            metrics.update_lists(logits=output[:, target_classes], y_true=batch_info['label'][:, target_classes])
